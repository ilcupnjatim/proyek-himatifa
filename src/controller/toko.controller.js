...
class TokoController extends TokoDB {
    ...
    async createDataToko(req, res, next) {
        try {
            ...
            if (data_id || toko_user_id) {
                ...
                let datas = await this.createToko(
                    name,
                    alamat,
                    toko_id,
                    req.user._id,
    ...
    async updateToko(req, res, next) {
        try {
            ...
            const data_id = await this.findTokoId(toko_id);
            if (data_id) {
                let up = await this.updateTokoDB(
                    name,
                    toko_id,
                    toko_id_update,
                    alamat,
    ...
    async claimCode(req, res, next) {
        try {
            ...
            if (check && check.claim == false) {
                let tokoNya = await new TransactionDB().getDataTransaction(
                ...
                if (isUser && isUser.owner == req.user._id) {
                    ...
                    let up = await this.updatePendapatan(
                        tokoNya.toko_id,
                        total_prices
                    );
                    if (up) {
                        let dataCode = await new TransactionDB().updateClaim(
                            code
                        );
                        const data =
                            await new TransactionDB().getDataTransaction(
                                dataCode.transaction_id
                            );
